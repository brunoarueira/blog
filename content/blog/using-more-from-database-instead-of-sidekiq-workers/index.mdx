...
With my hands full of data, I've started to analyze the feature that at the end fire the
aforementioned workers. The service object which handles all the logic to update those counters was
causing a lot of N+1 query, because they receives one id from the worker and simple call:
...
avoid to complete which each other, collaborating to another outage.
